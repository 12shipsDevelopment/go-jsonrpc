...
type wsConnMulti struct {
	// outside params
	conn             *websocket.Conn
	connFactory      func() (*websocket.Conn, error)
	connFactories     []func() (*websocket.Conn, error)
	currentFactoryIndex   int
...
func (c *wsConnMulti) nextMessage() {
	if c.timeout > 0 {
...
func (c *wsConnMulti) nextWriter(cb func(io.Writer)) {
	c.writeLk.Lock()
...
func (c *wsConnMulti) sendRequest(req request) error {
	c.writeLk.Lock()
	defer c.writeLk.Unlock()
...
func (c *wsConnMulti) handleOutChans() {
...
func (c *wsConnMulti) handleChanOut(ch reflect.Value, req int64) error {
...
func (c *wsConnMulti) handleCtxAsync(actx context.Context, id int64) {
	<-actx.Done()
...
func (c *wsConnMulti) cancelCtx(req frame) {
	if req.ID != nil {
...
func (c *wsConnMulti) handleChanMessage(frame frame) {
	var chid uint64
...
func (c *wsConnMulti) handleChanClose(frame frame) {
	var chid uint64
	if err := json.Unmarshal(frame.Params[0].data, &chid); err != nil {
		log.Error("failed to unmarshal channel id in xrpc.ch.val: %s", err)
		return
...
func (c *wsConnMulti) handleResponse(frame frame) {
...
func (c *wsConnMulti) handleCall(ctx context.Context, frame frame) {
	if c.handler == nil {
...
func (c *wsConnMulti) handleFrame(ctx context.Context, frame frame) {
...
func (c *wsConnMulti) closeInFlight() {
...
func (c *wsConnMulti) setupPings() func() {
	if c.pingInterval == 0 {
		return func() {}
	}
...
func (c *wsConnMulti) tryReconnect(ctx context.Context,index int) bool {
	if c.connFactories == nil { // server side
		return false
	}
	...
	go func() {
		...
		for conn == nil {
			...
			var err error
			if conn, err = c.connFactories[index](); err != nil {
...
func (c *wsConnMulti) handleWsConn(ctx context.Context) {
	...
	for {
		...
		case r, ok := <-c.incoming:
			...
			if ok {
				...
				if err == nil {
					...
					if ok1 {
						...
						if err !=nil{
							log.Errorf("websocket error", "DAEMONINDEX env err", err)
						}else{
							if index >= len(c.connFactories) {
								log.Errorf("websocket error", "DAEMONINDEX illegal")
							}else{
								if c.currentFactoryIndex != index { //关闭当前连接，准备开始切换
									c.conn.Close()
									c.currentFactoryIndex = index //重新连接新的index
								}
							}
						}
					}
			...
			if !c.tryReconnect(ctx,c.currentFactoryIndex) { //开始切换
		...
		case <-timeoutCh:
			...
			// The client performs the reconnect operation, and if it exits it cannot start a handlewsConnMulti again, so it does not need to exit
			continue
		case <-c.stop:
			c.writeLk.Lock()
